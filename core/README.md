...
- `src/runtime.zig` implements the in-memory JSON-RPC runtime for raw `memory.remember`, deterministic package-manager/test-command extraction, current-slot supersession, valid-at retrieval, `memory.search`, `memory.retrieve`, `memory.inspect`, `memory.forget`, `memory.feedback`, and `memory.core.*`.
